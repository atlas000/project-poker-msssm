...
The results showed, that a very passive strategy always lost. This corresponds to the outcome of a real heads up, where the passive player has a disadvantage due to the blinds. For two players in general the less extreme strategy showed to be more effective. However, this characteristic could not be verified to match reality. Concerning the learning algorithms, a significant increase in the average number of games won by the learning player could be observed. This indicates that with an adaptive strategy an important increase in the average games won can be achieved, also in real Texas Hold’em.\\
...
This section serves to understand how we implemented the simulation. Expressions are introduced that we use throughout the report. One can find the whole program on GitHub\footnote{https://github.com/atlas000/project-poker-msssm} in the folder code. Figure \ref{diagramm_waagrecht} gives an overview of all scripts used and how they are linked. \\

%
\begin{figure}[h]
\includegraphics[width=\textwidth]{Graphics/diagramm_waagrecht.pdf}
\label{diagramm_waagrecht}
\caption{Overview of of the scripts used}
\end{figure}
...
\subsubsection{adujstCardValue.m}
...
\begin{figure}[h]
\begin{center}
\includegraphics[scale=0.3]{Graphics/allDataPlot_BlindOn_1000Games_001Step_TopFlat.jpg}
...
\textbf{Advantages}
\begin{itemize}
\item With an increasing number of rounds the estimation continuously becomes better
\item On a long run he will always play the most suitable strategy and optimise his return
\item It reaches a good estimation of the opponent's risk factor already after a few rounds
\item The algorithm could also use the rounds where the other player has to pay the blind by counting the number of consecutive rounds he plays and applies the same strategy just by including the models standard deviation into the calculation. This would lead to a faster estimation of the opponent's risk factor
\end{itemize}
\end{minipage}\hfill
\begin{minipage}[t]{0.48\textwidth}
\textbf{Disadvantages}\\
...
\begin{figure}[h]
\includegraphics[width=\textwidth]{Graphics/OvViewLatex18}
\caption{Result-matrices for all three learning models. From left to right: threshold, count and gauge, iteration}
...
\section{Appendix}
The appendix and further files can be found on GitHub\footnote{https://github.com/atlas000/project-poker-msssm}.
